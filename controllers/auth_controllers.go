...
func (ctrl *AuthController) SignUpUser(ctx *gin.Context) {
	...
	if payload.Password != payload.PasswordConfirm {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": "password do not match"})
		return
	}
	...
	if result.Error != nil && strings.Contains(result.Error.Error(), "duplicate key value violates unique") {
		...
		ctx.JSON(http.StatusBadGateway, gin.H{"message": "something bad happend"})
		return
	}
...
func (ctrl *AuthController) SignInUser(ctx *gin.Context) {
	...
	result := ctrl.DB.First(&user, "email = ?", strings.ToLower(payload.Email))
	if result.Error != nil {
...
func (ctrl *AuthController) RefreshAccessToken(ctx *gin.Context) {
	...
	var user models.User
	result := ctrl.DB.First(&user, "id = ?", fmt.Sprint(sub))
	if result.Error != nil {
...
func (ctrl *AuthController) LogoutUser(ctx *gin.Context) {
